import { eq, and, desc, asc, ilike, count, sql, ne, gte, lt, not, like } from "drizzle-orm";
import { 
  users, 
  pdfFiles, 
  purchases, 
...
export class DbStorage implements IStorage {
  ...
  async getAdminStats(): Promise<{
    ...
    const [currentMonthUsers] = await db.select({ count: count() }).from(users)
      .where(gte(users.createdAt, lastMonthStart.toISOString()));
    ...
    const [currentMonthFiles] = await db.select({ count: count() }).from(pdfFiles)
      .where(gte(pdfFiles.createdAt, lastMonthStart.toISOString()));
  ...
  async getAdminActivities(limit = 50): Promise<ActivityLog[]> {
  ...
  async createBankCard(data: InsertBankCard): Promise<BankCard> {
    return await db.transaction(async (tx) => {
      ...
      if (data.isDefault) {
        await tx.update(bankCards).set({ isDefault: false });
      }

      const [card] = await tx.insert(bankCards).values(data).returning();
      return card;
    });
  }
  ...
  async updateBankCard(id: number, data: Partial<InsertBankCard>): Promise<BankCard | undefined> {
    return await db.transaction(async (tx) => {
      ...
      if (data.isDefault) {
        await tx.update(bankCards).set({ isDefault: false }).where(ne(bankCards.id, id));
      }

      const [card] = await tx.update(bankCards)
        .set(data)
