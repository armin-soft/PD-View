...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/bank-cards", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "خطا در ایجاد کارت بانکی" });
    }
  });
  ...
  app.put("/api/admin/bank-cards/:id", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "خطا در به روزرسانی کارت بانکی" });
    }
  });
  ...
  app.delete("/api/admin/bank-cards/:id", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "خطا در حذف کارت بانکی" });
    }
  });
